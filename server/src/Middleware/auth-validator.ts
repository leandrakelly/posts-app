...
const isProtectedRoute = (query: string): boolean => { 
  const { definitions: [operation] } = parse(query);
  const route = operation.kind === 'OperationDefinition' ? (operation.selectionSet.selections[0] as FieldNode).name.value : '';
...
const authValidator = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
